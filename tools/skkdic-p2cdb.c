...
main(int argc, char **argv)
{
  ...
  if (cdb_make_start(&cdb, fd) == -1) {
    fprintf(stderr, "cdb_make_start() failed.\n");
    return 3;
  }
  ...
  while (fgets(buf, LINE_SIZE, stdin)) {
    ...
    if (cdb_make_addbegin(&cdb, strlen(wbuf), strlen(rbuf)) == -1) {
      fprintf(stderr, "cdb_make_addbegin() failed.\n");
      return 5;
    }
    if (buffer_puts(&cdb.b, wbuf) == -1) {
      fprintf(stderr, "buffer_puts() failed.\n");
      return 6;
    }
    h = CDB_HASHSTART;
    ...
    if (cdb_make_addend(&cdb, strlen(wbuf), strlen(rbuf), h) == -1) {
      fprintf(stderr, "cdb_make_addend() failed.\n");
      return 8;
    }
  }

  if (cdb_make_finish(&cdb) == -1) {
    fprintf(stderr, "cdb_make_finish() failed.\n");
    return 9;
  }
  if (close(fd) == -1) {
